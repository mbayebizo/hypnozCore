...
public class GroupesServices {
	...
	public GroupesServices(GroupesRepository groupesRepository, GroupesMapper groupesMapper, StructuresRepository structuresRepository) {
	...
	public GroupesDto save(GroupesDto groupesDto) {
		...
		Structures structures = structuresRepository.findById(groupesDto.getStructures().getId()).orElseThrow(() -> {
			log.debug("Not found STRUCTURE"+groupesDto.getStructures());
			return new ResponseException(RequestErrorEnum.NOT_FOUND_STRUCTURE);
		});
		groupes.setStructures(structures);
		groupes = groupesRepository.saveAndFlush(groupes);
	...
	public List<GroupesDto> listByStructure(Long sid) {
		try {
			List<Groupes> groupeList = groupesRepository.findAll().stream().filter(grp -> grp.getStructures().getId().equals(sid)).toList();
	...
	public void deleteById(Long id) {
		try {
			groupesRepository.findById(id);
			groupesRepository.deleteById(id);
			LOGGER.debug(GroupesServices.class.getName(), HypnozCoreCostance.DELETED, id);
		}catch (ResponseException e) {
			log.error(e.getMessage(), e);
            throw new ResponseException(RequestErrorEnum.NOT_FOUND_GROUPE);
		}
